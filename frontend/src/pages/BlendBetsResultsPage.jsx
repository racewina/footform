...
async function fetchBlendResults(date) {
  const res = await fetch(`/api/blend-bets/results?date=${date}&tz=${encodeURIComponent(TZ)}`, {
...
export default function BlendBetsResultsPage() {
  const [date, setDate] = useState(() => yesterday());
  const dateStr = ymd(date);
  const isToday = dateStr === ymd(startOfToday());

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["blend-results", dateStr],
    queryFn: () => fetchBlendResults(dateStr),
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.note}>
        ...
        <span>
          How the blended slips would have done. We rebuilt each slip from that
          day's finished matches — predictions from each team's form <em>before</em>{" "}
          kickoff, priced at the bookmaker odds that were available (1.20+ only) —
          and graded every leg. A slip wins only if <strong>all</strong> legs land.
        </span>
      </div>
      ...
      <div style={styles.list}>
        ...
        {!isLoading && !isError && !hasSlips && (
          <p style={styles.empty}>No qualifying finished matches to build a slip from on this date.</p>
        )}
        {!isLoading && !isError && hasSlips &&
...
function SlipCard({ slip }) {
  const { target, legs, combinedBookOdds, legCount, legHits, won } = slip;
    ...
    <div style={styles.leg}>
      ...
      <div style={styles.legMain}>
        ...
        <span style={{ ...styles.legSelection, color: leg.hit ? "var(--accent)" : "var(--text3)" }}>
          {leg.selection}
          {leg.bookmaker && <span style={styles.legBook}> · {leg.bookmaker}</span>}
        </span>
      </div>
      <div style={styles.legNums}>
        <span style={styles.legScore}>{leg.homeScore}–{leg.awayScore}</span>
        <span style={styles.legOdds}>{leg.bookOdds.toFixed(2)}</span>
      </div>
    </div>
  );
}
...
const styles = {
  ...
  legBook: { fontSize: 11, fontWeight: 400, color: "var(--text3)" },
